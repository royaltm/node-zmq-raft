    ...
    if (prevIndex < log.lastIndex) {
      ...
      if (manyEntriesAtOnce) {
        ...
        .then(entries => {
          ...
          debug('updating follower %s with entries: %s - %s term: %s', followerId, followerNextIndex, lastIndex, lastTerm);
          return payload;
        });
      } else {
        ...
        .then(entry => {
          ...
          payload.push(entry);
          debug('updating follower %s with entry: %s term: %s', followerId, lastIndex, lastTerm);
          return payload;
        });
      }
    }
