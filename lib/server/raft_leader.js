...
exports.onElectionTimeout = function() {
  ...
  .then(() => {
    ...
    for(let [peerId, rpc] of this._rpcs) {
      ...
      .catch(err => {
        if (!err.isCancel) this.emit('error', err);
      });
    }
...
function onVoteResponse(currentTerm, peerId, term, voteGranted) {
  ...
  else if (this.currentTerm === currentTerm && this.state === FSM_CANDIDATE && !this.voted.has(peerId)) { // sanity check
    this.voted.add(peerId);
    let votes = ++this.votes;
    debug("id: %s vote: %s yes: %s / %s  <?>  %s", peerId, voteGranted, votes, this.voted.size + 1, this.majority);
    if (voteGranted && votes >= this.majority) {
...
function becomeLeader() {
  ...
  for(let [followerId, rpc] of this._rpcs) {
    ...
    .then(args => onAppendEntriesResponse.call(this, currentTerm, followerId, lastIndex, lastTerm, ...args))
    .catch(err => {
      if (!err.isCancel) this.emit('error', err);
    });
  }
...
function onAppendEntriesResponse(currentTerm, followerId, sentPrevIndex, sentPrevTerm, term, success, conflictTerm, conflictTermIndex) {
  ...
  if (term > this.currentTerm) {
    debug("append entries response term: %s > %s", term, this.currentTerm);
    ...
    if (success) {
      followerNextIndex = sentPrevIndex + 1;
      // • If successful: update nextIndex and matchIndex for follower (§5.3)
      matchIndex.set(followerId, sentPrevIndex);
      nextIndex.set(followerId, followerNextIndex);
      // If there exists an N such that N > commitIndex, a majority
      // of matchIndex[i] ≥ N, and log[N].term == currentTerm:
      // set commitIndex = N (§5.3, §5.4).
      if (sentPrevTerm === currentTerm && sentPrevIndex > this.commitIndex) {
        let majority = this.majority;
        for(let id of this.peers.keys()) {
          if (matchIndex.get(id) >= sentPrevIndex && (--majority) === 1) {
            this.commitIndex = sentPrevIndex;
...
function sendAppendEntries(followerId, followerNextIndex, manyEntriesAtOnce) {
  ...
  return log.termAt(prevIndex).then(term => {
    ...
    if (prevIndex < log.lastIndex) {
      ...
      if (manyEntriesAtOnce) {
        return log.readEntries(prevIndex, log.lastIndex, buffer)
        .then(entries => {
          const last = entries.length - 1;
          prevIndex += last;
          prevTerm = log.readTermOf(entries[last]);
          push.apply(payload, entries);
          debug('updating follower %s with entries: %s - %s term: %s', followerId, prevIndex - last, prevIndex, prevTerm);
          return payload;
        });
      } else {
        return log.getEntry(prevIndex, buffer)
        .then(entry => {
          prevTerm = log.readTermOf(entry);
          payload.push(entry);
          debug('updating follower %s with entry: %s term: %s', followerId, prevIndex, prevTerm);
          return payload;
        });
      }
    }
  ...
  .then(payload => {
    ...
    .then(args => onAppendEntriesResponse.call(this, currentTerm, followerId, prevIndex, prevTerm, ...args))
  })
  .catch(err => {
    if (!err.isCancel) this.emit('error', err);
  });
...
function sendInstallSnapshot(currentTerm, followerId, snapshot, position) {
  ...
  return snapshot.read(position, length, buffer, 0).then(data => {
    if (currentTerm !== this.currentTerm || this.state !== FSM_LEADER) return; // sanity check
    ...
    .then(([term, reposition]) => {
      if (done && reposition === undefined) {
        onAppendEntriesResponse.call(this, currentTerm, followerId, lastIncludedIndex, lastIncludedTerm, term, true);
      }
