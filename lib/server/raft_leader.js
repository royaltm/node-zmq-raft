...
exports.onElectionTimeout = function() {
  ...
  if (this.state === FSM_CANDIDATE && !cluster.majorityHasVoted()) {
      ...
      if (sentLastTerm === currentTerm && sentLastIndex > this.commitIndex) {
        ...
        if (cluster.majorityHasLogIndex(sentLastIndex, matchIndex)) {
          ...
          if (!cluster.isMember(this.peerId)) {
            ...
            const peersUpdateRequest = this._persistence.peersUpdateRequest
                , configLogIndex = this._log.getRid(peersUpdateRequest);

            if (this.commitIndex >= configLogIndex) {
              ...
              const peers = cluster.serializeNC();
              this._updateConfig(peers);
              return this._persistence.update({peers})
