  ...
  **/
  constructor(peerId, persistence, log, stateMachine, options) {
    ...
    Promise.all([log.ready(), persistence.ready(), stateMachine.ready()]).then(() => {
      var {currentTerm, votedFor, peers, peersUpdateRequest} = persistence;
      ...
      if (this.cluster.isTransitional) {
        if (peersUpdateRequest == null) {
          throw new Error("ZmqRaft: sanity error - transitional config without request key");
        }
        if (!log.getRid(peersUpdateRequest)) {
          throw new Error("ZmqRaft: sanity error - transitional config not found in the log or request config update has expired");
        }
      }
  ...
  //                       CANDIDATE -> LEADER
  //                                    LEADER -> FOLLOWER
  _updateState(state, currentTerm, votedFor, peers, peersUpdateRequest) {
    ...
    if (peers !== undefined) this._updateConfig(peers);
    ...
    if (prevTerm !== currentTerm || this.votedFor !== votedFor) {
      ...
      return this._persistence.update({votedFor, currentTerm, peers, peersUpdateRequest});
    }
    else {
      if (prevState !== state) this._emitState();
      if (peers !== undefined) {
        return this._persistence.update({peers, peersUpdateRequest});
      }
