...
class ZmqRaft extends ReadyEmitter {
   ...
   * - maxLogEntryDataSize=10485760 {number}: maximum log entry data bytes (hard limit)
   *                                          enforced by RequestUpdate server handler
   * - peerMsgDataSize=65536 {number}: hint how many bytes are send to followers via each
  ...
  **/
  constructor(peerId, persistence, log, stateMachine, options) {
    ...
    this._rndElectionTimeout = createRangeRandomizer(
        validateIntegerOption(options, 'electionTimeoutMin', 100),
        validateIntegerOption(options, 'electionTimeoutMax', 100));
