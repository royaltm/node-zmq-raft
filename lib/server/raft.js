...
class ZmqRaft extends ReadyEmitter {
   ...
   * - arbiter=false {boolean}: Select whether this instance should run in ARBITER mode.
   *                            An ARBITER can never become a CANDIDATE.
   * - electionTimeoutMin=200 {number}: An optional election min timeout, ignored in ARBITER mode.
   * - electionTimeoutMax=300 {number}: An optional election max timeout, ignored in ARBITER mode.
  ...
  **/
  constructor(peerId, persistence, log, stateMachine, options) {
    ...
    if (options.arbiter) {
      debug('ARBITER mode enabled');
      this._refreshElectionTimeout = this._clearElectionTimeout;
    }
