...
const { lastConfigEntryOf, readers: { readDataOf, readRequestIdOf } } = require('../common/log_entry');
...
function requestVoteHandler(reply, candidateId, candidateTerm, candidateLastIndex, candidateLastTerm) {
  ...
  else if (this.state === FSM_CANDIDATE && !this.cluster.majorityHasVoted()) {
...
function appendEntryHandler(reply, leaderId, leaderTerm, leaderPrevIndex, leaderPrevTerm, leaderCommit, ...entries) {
  ...
  if (leaderTerm < currentTerm) {
    ...
    .then(term => {
      ...
      if (term === leaderPrevTerm) config = lastConfigEntryOf(entries);
