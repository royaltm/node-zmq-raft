...
const { decodeRequestId } = require('../server/zmq_rpc_socket');
    ...
    if (typeLength === 1) {
      switch(msgType[0]) {
        ...
        case APPEND_ENTRY_MATCH:
          args = decodeAppendEntriesRequest(args).slice(2);
          appendEntryHandler.call(this, reply, ...args);
          return;
        case INSTALL_SNAPSHOT_MATCH:
          args = decodeInstallSnapshotRequest(args).slice(2);
          installSnapshotHandler.call(this, reply, ...args);
          return;
        case REQUEST_VOTE_MATCH:
          args = decodeRequestVoteRequest(args).slice(2);
          requestVoteHandler.call(this, reply, ...args);
          return;

        /* clients */
        case REQUEST_UPDATE_MATCH:
          args = decodeRequestUpdateRequest(args).slice(2);
          requestUpdateHandler.call(this, reply, ...args);
          return;
        case REQUEST_ENTRIES_MATCH:
          args = decodeRequestEntriesRequest(args).slice(2);
          requestEntriesHandler.call(this, reply, ...args);
          return;
        case REQUEST_CONFIG_MATCH:
          args = decodeRequestConfigRequest(args).slice(2);
          requestConfigHandler.call(this, reply, ...args);
          return;
        case REQUEST_LOG_INFO_MATCH:
          args = decodeRequestLogInfoRequest(args).slice(2);
          requestLogInfoHandler.call(this, reply, ...args);
          return;
      }
    }
    if (typeLength >= 1) {
...
function bounceDuplicateRequestCache(rpcLastRequestIds, peerId, reply) {
  const requestId = decodeRequestId(reply.requestId)
      , ident = reply.ident.toString('base64');

  var cache = rpcLastRequestIds.get(peerId);

  if (cache.ident === ident && cache.requestId === requestId) {
    /* bounce */
