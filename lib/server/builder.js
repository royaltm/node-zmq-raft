...
function createFileLog(options) {
  const raftdir = options.data.path
      ...
      , { indexFileCapacity
        , requestIdTtl } = options;

  if (indexFileCapacity !== undefined) debug('index file capacity: %j', indexFileCapacity);
  if (requestIdTtl !== undefined) debug('request id TTL: %j ms', requestIdTtl);

  return new FileLog(logdir, snapfile, {indexFileCapacity, requestIdTtl});
}
...
exports.build = function build(options) {
  ...
  .then(([persistence, log, stateMachine]) => {
    ...
    if (label != null) debug('cluster label: %s', label);
