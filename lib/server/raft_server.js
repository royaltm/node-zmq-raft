...
exports.requestUpdateHandler = function(reply, logData) {
  ...
  /* request seen and already applied  */
    ...
    debug('request update index already applied');
    respArg = index;
    sendReply();
  }
...
exports.requestEntriesHandler = function(reply, prevIndex, count) {
  ...
  const pipe = () => {
    debug('pipe: nextIndex: %s prevIndex: %s lastIndex: %s nextToSendIdx: %s', nextIndex, prevIndex, lastIndex, nextToSendIdx);
    ...
    debug('pipe after: nextIndex: %s prevIndex: %s lastIndex: %s nextToSendIdx: %s queue.length: %s', nextIndex, prevIndex, lastIndex, nextToSendIdx, queue.length);
  ...
  const request = (installSendReply, prevIndex, count) => {
    debug('request: prevIndex: %s count: %s queue: %s nextToSendIdx: %s', prevIndex, count, queue.length, nextToSendIdx);
