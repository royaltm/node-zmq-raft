...
exports.configUpdateHandler = function(reply, peers) {
  ...
  if (index === undefined) {
    ...
    this._updateConfig(peers);
    return this._persistence.update({peers, peersUpdateRequest: requestKey})
    ...
    .then(index => {
      if (currentTerm === this.currentTerm && this.state === FSM_LEADER) { /* sanity check */
        if (index <= this.lastApplied) {
          ...
          debug('appended Cold,new [%s] to log and commited at index: %s', requestKey, index);
      ...
      else debug('ConfigUpdate: leader stepped down before replying to a client');
    })
...
exports.requestLogInfoHandler = function(reply) {
  const isLeader = this.state === FSM_LEADER
      ...
      , leaderId = isLeader ? this.peerId
                            : this.followLeaderId
      , firstFreshIndex = log.getFirstFreshIndex();
  ...
  reply(encodeRequestLogInfoResponse([
    ...
    firstFreshIndex === undefined ? log.lastIndex
                                  : firstFreshIndex - 1
  ]));
}
