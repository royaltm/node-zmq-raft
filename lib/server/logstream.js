...
class LogStream extends Readable {
  constructor(fileLog, firstIndex, lastIndex, options) {
    ...
    this.maxChunkEntries = (options.maxChunkEntries >>> 0) || 8192;
    this.maxChunkSize = (options.maxChunkSize >>> 0) || 128*1024;
    ...
    if (logStream && 'object' === typeof logStream) {
      ...
      return this.fileLog._indexFileOf(index).then(indexFile => {
        ...
        var input = []
          ...
          , logStream = this._logStream = indexFile.createLogReadStream(index, lastIndex)
          , unref = () => {
            if (indexFile) {
