...
class WebMonitor {
   ...
   * options:
   *
   * - port {number}: a webmonitor (external interface, announced) port
   * - host {string}: a webmonitor (external interface, announced) host
   * - bind {Object}: optional server bind options: host {string} and port {number}
  ...
  **/
  constructor(zmqRaft, { port=8050
    ...
    this.url = formatUrl({protocol:'http', hostname: host, port});
    ...
    const listen = () => {
      debug('binding to: [%s]:%s', host, port);
