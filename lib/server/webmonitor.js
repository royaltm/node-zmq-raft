...
class WebMonitor {
   ...
   *
   * WebMonitor instance will bind to the provided tcp address and will close
   * itself once the `zmqRaft` instance is closed.
   *
   * options:
   *
   * - port {number}: a webmonitor listener (external interface) port
   * - host {string}: a webmonitor listener bind host
   * - bind {Object}: optional bind options
  ...
  **/
  constructor(zmqRaft, { port=8050
    ...
    if (bind !== null && 'object' === typeof bind) {
    ...
    else if ('string' === typeof bind) {
      host = bind;
    }
    else if ('number' === typeof bind) {
      port = bind;
    }
  ...
  **/
  ...
  _configData() {
    return {peers: this.zmqRaft.peersAry, port: this.port, label: this.zmqRaft.label};
  }

  _configPayload() {
    return {event: "config", data: _configData()};
  }
