...
const debug = require('debug')('broadcast-state-machine');
...
class BroadcastStateMachine extends FilePersistence {
  ...
  **/
  constructor(filename, url, options) {
    ...
    var fan = this._fan = zmq.socket('pub');
    this._fan.setsockopt(zmq.ZMQ_LINGER, 2000);
  ...
  applyEntries(entries, nextIndex, currentTerm, snapshot) {
    return synchronize(this, () => {
      ...
      if (nextIndex <= lastApplied) {
        if (nextIndex <= lastApplied || !snapshot || snapshot.logIndex + 1 !== nextIndex) {
