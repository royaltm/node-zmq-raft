...
class FileLog extends ReadyEmitter {
   ...
   * - requestIdTtl {number}: an updating request id Time-To-Live in milliseconds
   *
   * @return {FileLog}
  **/
  constructor(logdir, snapshot, options) {
    ...
    if (!isPowerOfTwo32(indexFileCapacity)) {
      throw new Error("indexFileCapacity should be a power of 2");
    }
    defineConst(this, 'requestIdTtl', validateIntegerOption(options, 'requestIdTtl', MIN_REQUEST_ID_TTL, MAX_REQUEST_ID_TTL));
