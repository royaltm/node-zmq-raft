...
const zeroRequestBuf = Buffer.alloc(12, 0)
...
const { REQUEST_LOG_ENTRY_OFFSET
      ...
      , isRequestExpired
      , mixinReaders } = require('../common/log_entry');
...
const logEntryTypeStateBuf = Buffer.from([LOG_ENTRY_TYPE_STATE]);
...
class FileLog extends ReadyEmitter {
  ...
  getRid(requestId) {
    if ('string' === typeof requestId && requestId.length === 16) {
      return this[requestDb$].get(requestId);
    }
    else if (isBuffer(requestId) && requestId.length === 12) {
  ...
  appendCheckpoint(term) {
    ...
    return synchronize(this, () => {
      var buffer = this._checkpointLogEntryBuf;
      if (buffer === undefined) {
        buffer = this._checkpointLogEntryBuf = Buffer.allocUnsafe(LOG_ENTRY_HEADER_SIZE + 1);
        buffer.fill(0, REQUEST_LOG_ENTRY_OFFSET, TYPE_LOG_ENTRY_OFFSET);
        buffer[TYPE_LOG_ENTRY_OFFSET] = LOG_ENTRY_TYPE_CHECKPOINT;
        buffer[LOG_ENTRY_HEADER_SIZE] = NULL_MSGPACKED_CODE;
      }
      buffer.writeUIntLE(term, TERM_LOG_ENTRY_OFFSET, 7, true);

      debug('applying checkpoint with term: %s', term);

      return this._lastIndexFile()
      .then(indexFile => synchronize(indexFile, () => this._append(indexFile, buffer, term)));
    });
  }
  ...
  appendEntries(entries, index) {
    ...
    return synchronize(this, () => {
      ...
      return this._truncate(index).then(() => {
        ...
        debug('applying entries %s at index: %s', numEntries, index);

        var write = (indexFile, index, entries) => synchronize(indexFile, () => indexFile.writev(entries, index)
        .then(([numUnwritten, nextEntry]) => {
          var lastWritten = nextEntry - index - 1;
          this[lastIndexFile$] = indexFile;
          if (lastWritten >= 0) {
            for(var i = 0; i <= lastWritten; ++i) {
              var req = entries[i].toString('base64', 0, 12);
              if (req !== zeroRequestStr) rdb.set(req, index + i);
            }
          ...
          else debug('applied entries %s lastIndex: %s lastTerm: %s', numEntries, this.lastIndex, this.lastTerm);
        }));
  ...
  _readRequestsFromLogs() {
    ...
    var rdb = this[requestDb$];
    var numExpired = 0;
    var requestId = Buffer.allocUnsafe(12);
    var start = Date.now();
    var read = (indexFile, index) => {
      if (indexFile.includes(index)) {
        return indexFile.readSlice(index, 0, 12, requestId).then(() => {
          if (!requestId.equals(zeroRequestBuf)) {
            if (isRequestExpired(requestId)) {
              if (++numExpired > indexFile.capacity) {
                ...
                debug("read: %s requests in %s seconds, no more fresh entries", rdb.size, ((Date.now() - start) / 1000).toFixed(1));
                return;
              }
            }
            else {
    ...
    return read(lastIndexFile, this.lastIndex);
  }
