...
class FileLog extends ReadyEmitter {
  ...
  **/
   ...
   *
   * resolves to undefined or the last config entry found in entries (whole entry as a Buffer data)
   *
        ...
        .then(([numUnwritten, nextEntry]) => {
          ...
          if (lastWritten >= 0) {
            for(i = 0; i <= lastWritten; ++i) {
              ...
              if (readTypeOf(entry) === LOG_ENTRY_TYPE_CONFIG) {
                lastConfigEntry = entry;
              }
