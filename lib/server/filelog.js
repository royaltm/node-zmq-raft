...
class FileLog extends ReadyEmitter {
  ...
  **/
  ...
  /**
   * returns log entry index for given request id
   *
   * requestId must be fresh
   *
  ...
  appendState(requestId, term, data) {
    ...
    return synchronize(this, () => {
      ...
      return this._lastIndexFile(indexFile => {
        ...
        debug('appending state entry: (%s) with term: %s', data.length, term);
    ...
    return synchronize(this, () => {
      ...
      return this._truncate(index).then(() => {
        ...
        .then(([numUnwritten, nextEntry]) => {
          ...
          else debug('appended log (%s) indexes: %s - %s last term: %s', numEntries, firstIndex, this.lastIndex, this.lastTerm);
        });
    ...
    .then(created => {
      ...
      const installer = () => {
        if (checkWatcher()) {
          ...
          .then(() => checkWatcher() && setTimeout(startWatching, 10000).unref())
          .catch(err => {
            console.error('FileLog: install snapshot failed to open: %s', err);
            checkWatcher() && setTimeout(startWatching, 10000).unref();
          });
        }
      };
