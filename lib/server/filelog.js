...
class FileLog extends ReadyEmitter {
  ...
  **/
  ...
  _getTermBufferCached(term) {
    ...
    termBuffer.writeUIntLE(term, 0, 7, true);
    return termBuffer;
  }
    ...
    return this._truncate(index).then(() => {
      ...
      .then(([numUnwritten, nextEntry]) => {
        ...
        if (lastWritten >= 0) {
          ...
          if (entry.length === 4) {
            this.lastTerm = entry[2].readUIntLE(0, 7, true);
          }
          else {
            this.lastTerm = entry.readUIntLE(TERM_LOG_ENTRY_OFFSET, 7, true);
          }
        }
        if (numUnwritten !== 0) {
  ...
  **/
  ...
  _indexFileNameOf(index) {
    if (index > this.lastIndex) return Promise.resolve();
    var basename = logBaseName(index);
    var prefix = basename.substr(0, INDEX_PATH_PREFIX_LENGTH);
  ...
  static readIndexFileNames(logdir) {
