...
exports.parsePeers = function(peers) {
  ...
  return peers.reduce((map, peer) => {
    ...
    if (isNonEmptyString(peer)) {
      ...
      throw new TypeError("peer must be an url string or tuple [id, url] or object with 'url' and 'id' properties");
    }
    else {
      id = peer.id
    ...
    if (!isNonEmptyString(id)) throw new TypeError("peer must have id string that is not empty");
    if (!isNonEmptyString(peer)) throw new TypeError("peer must have url string that is not empty");
    map.set(id, peer);
