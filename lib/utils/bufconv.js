 ...
 * @return {Buffer}
**/
function allocBufNumberLE(value) {
...
function readBufUIntLE(buffer, offset, stop) {
  ...
  if (stop != null && limit > (stop = (stop >> 0) - 1)) limit = stop;
...
function readBufNumberLE(buffer, offset, stop) {
  ...
  if (stop != null && length > (stop = stop >> 0)) length = stop;
  length -= offset;
