...
/*
protocol = new RpcProtocol(schema, options)

schema = ['string', 'number', 'boolean', 'binary', 'hex', 'base64', 'object', 'uint']
options = {required: 3}

rpc = protocol.createRpc(url, options)

[a, b, c] = protocol.decode(args)
[a, b, c] = protocol.encode(args)


router = protocol.createRouter(bindUrl, handler)

handler = function()

rpc.request(a, b, c).then([e,f,g] => {
...
class FramesProtocol {
  ...
  **/
  constructor(requestSchema, responseSchema, options) {
    ...
    this.name = String(options.name || 'unnamed protocol');
  ...
  **/
  ...
  /**
   * Creates specialized router message listener and attaches it to 'message' event
   *
  ...
  **/
  createRouterMessageListener(router, handler, context) {
    if (!(router instanceof EventEmitter)) throw new TypeError("${this.name}: router must be an instance of EventEmitter");
    if ("function" !== typeof router.send) throw new TypeError("${this.name}: router must have send method");

    const name = this.name;
    const decodeRequest = this.decodeRequest;
    const encodeResponse = this.encodeResponse;

    var listener = (ident, requestId, ...args) => {
      if (ident === undefined) throw new Error("${name}: router received message without ident frame")
      if (requestId === undefined) throw new Error("${name}: router received message without requestId frame")
    ...
    router.on('message', listener);

    return listener;
  }

  /**
   * Creates specialized subscriber message listener and attaches it to 'message' event
   *
  ...
  **/
  createSubMessageListener(sub, handler, context) {
    if (!(sub instanceof EventEmitter)) throw new TypeError("${this.name}: sub must be an instance of EventEmitter");
  ...
  **/
  createSendFunctionFor(pub) {
    if ("function" !== typeof pub.send) throw new TypeError("${this.name}: pub must have send method");
    const encodeRequest = this.encodeRequest;
    /* create unbound request function */
