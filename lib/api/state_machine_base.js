...
There are 2 special events emitted on state machine from Raft:

When raft state changes:

"raft-state": (state, currentTerm)
- `state{number}`: peer's current state (see common.constants.FSM_*)
- `currentTerm{number}`: peer's current term

When client request type is unknown to raft:
...
Client's rpc requests sent to the raft's ZMQ_ROUTER follows Dispatch RPC. The msgType is the 1st frame,
while the msgArgs are frames beginning with 3rd frame.
The 2nd frame is authorization frame and is discarded from this event args.
...
They might help to bounce duplicated messages as for these both properties should have unchanged values.
...
class StateMachineBase extends ReadyEmitter {
  ...
  **/
   ...
   * `nextIndex` - first log index of the first entry
   * `snapshot` - snapshot to reset state machine from
   * 
  ...
  **/
  applyEntries(entries, nextIndex, currentTerm, snapshot) {
    ...
    assert(nextIndex <= this.lastApplied || snapshot && snapshot.logIndex + 1 === nextIndex);
