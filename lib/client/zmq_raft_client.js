...
class ZmqRaftClient extends ZmqProtocolSocket {
  ...
  **/
  requestConfig(timeout) {
    ...
    const msg = [requestConfigTypeBuf, this[secretBuf$]]
      ...
      .catch(err => {
        if (err.isTimeout && (timeout === 0 || now() < expire)) {
  ...
  **/
   ...
   * (status, entries or chunk{Array|Buffer}, lastIndex{number}[, byteOffset{number}, snapshotSize{number}]) => bool (=== false means stop)
   *
            ...
            onresponse: (res, reply, refresh) => {
              ...
              case RE_STATUS_SNAPSHOT:
                ...
                if (status === RE_STATUS_SNAPSHOT) {
                  ...
                  snapshotSize = arg[1];
                  chunk = res[3];
                  if (currentByte !== byteOffset) throw new Error("ZmqRaftClient.requestEntries: snapshot chunks not in order");
                  currentByte = byteOffset + chunk.length;
                  ret = receiver(status, chunk, lastIndex, byteOffset, snapshotSize);
                  status = (byteOffset + chunk.length === snapshotSize) ? RE_STATUS_LAST
                                                                        : RE_STATUS_MORE;
                }
                else {
                  res.splice(0, 3);
                  if (lastIndex - res.length !== prevIndex) throw new Error("ZmqRaftClient.requestEntries: entries not in order");
                  prevIndex = lastIndex;
