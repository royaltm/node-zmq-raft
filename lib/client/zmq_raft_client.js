...
class ZmqRaftClient extends ZmqProtocolSocket {
   ...
   * - `secret` {String|Buffer}: a secret to send to servers
   ...
   * - `heartbeat` {number}: how often in milliseconds update cluster configuration via request config rpc;
   *   to disable heartbeats pass 0 (default)
    ...
    const msg = [requestConfigTypeBuf, this[secretBuf$]]
      ...
      .then(([isLeader, leaderId, peers]) => {
        ...
        if (isLeader) {
          ...
          /* not a leader */
          return this.leaderId === null ? this.delay(SERVER_ELECTION_GRACE_MS).then(request)
                                        : request();
        }
      })
      .catch(err => {
    ...
    const msg = [requestLogInfoTypeBuf, this[secretBuf$]]
      ...
      .then(res => {
        if (!anyPeer && !res[0]) {
          ...
          if (expire !== undefined && now() >= expire)
            throw new TimeoutError();
          return this.leaderId === null ? this.delay(SERVER_ELECTION_GRACE_MS)
    ...
    const msg = [requestEntriesTypeBuf, this[secretBuf$], lastIndex, count]
      ...
      .then(result => {
        ...
        if (this.leaderId === null) { /* election in progress */
          ...
          return this.delay(SERVER_ELECTION_GRACE_MS)
                .then(() => this.requestConfig(timeout)).then(request);
        }
        else return request();
      })
    ...
    const request = () => client.request(msg, options)
      .then(result => {
        ...
        if (client.leaderId === null) { /* election in progress */
          options.timeout = timeoutMs; /* reset timeout */
          return client.delay(SERVER_ELECTION_GRACE_MS)
