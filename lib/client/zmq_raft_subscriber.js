...
const { CLIENT_HEARTBEAT_INTERVAL
      , RE_STATUS_SNAPSHOT
...
const DEFAULT_BROADCAST_TIMEOUT = CLIENT_HEARTBEAT_INTERVAL * 2;
...
class ZmqRaftSubscriber extends Duplex {
   ...
   *
   * - `url` {String}: an url to fetch peer descriptors from
   * - `urls` {Array}: an array of urls to fetch peer descriptors from
   * - `peers` {Array}: array of raft server (peer) descriptors
   * - `secret` {String}: a secret to send to servers and subscribe to
   * - `lastIndex` {number}: index of last entry in local state machine
   * - `broadcastTimeout` {number}: milliseconds before the broadcast server is considered gone
   *   default is CLIENT_HEARTBEAT_INTERVAL * 2 (= 1 second)
   * - `timeout` {number}: time in milliseconds after which we consider raft server
   *    as dead and we'll try another one, default is SERVER_RESPONSE_TTL (= 0.5 second)
   ...
   *   default is 1
   * - `duplex` {Object}: super class initialization options (stream.Duplex, e.g. highWaterMark)
   * - `readable` {Object}: RequestEntriesStream super class initialization (stream.Readable, e.g. highWaterMark)
   *
  ...
  **/
  constructor(urls, options) {
    ...
    if (broadcastTimeoutMs < CLIENT_HEARTBEAT_INTERVAL) {
      broadcastTimeoutMs = CLIENT_HEARTBEAT_INTERVAL;
    }

    const client = new ZmqRaftClient(urls, Object.assign({lazy: true}, options));

    super(Object.assign({highWaterMark: 4}, options.duplex, {objectMode: true}));
