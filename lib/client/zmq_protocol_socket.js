...
const { ZMQ_LINGER, ZMQ_SNDHWM } = require('zmq');
...
class ZmqProtocolSocket extends ZmqBaseSocket {
      ...
      if (onresponse !== undefined) {
        ...
        try {
          result = onresponse(args, (msg) => {
            if (protocol !== undefined) msg = protocol.encodeRequest(msg);
            this[send$](requestKey, [requestId].concat(msg));
          }, handler.refresh);
        } catch(err) {
          handler.reject(err);
        }
  ...
  **/
  ...
  /* try sending immediately, when unsuccessfull queue, may throw */
  [send$](requestKey, payload) {
    const queues = this[queues$]
        , isFlushing = this[isFlushing$];

    if (isFlushing || queues.size !== 0) {
