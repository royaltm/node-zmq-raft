...
Index File Format is an extension to the token file format (see utils/tokenfile).

The file consists of tagged chunks (segments).
The 1st segment's header token is "RLOG" optionally followed by the "META" or
more segments followed by the "ITMZ" empty segment marker followed by log entries data.

The RLOG segment contains information about the first entry index, file capacity and
offsets of all stored log entries:
...
The marker:
  ...
  0 | "ITMZ"
  4 | 0x00000000
  8 | the beginning of the log entries data
...
On appending, the entry data is being written first, then the offset pointing after the entry is written at index table, thus committing write
On rollback only offset table is being zeroed - starting from the index being rolled back to.
There's an additional safety step before committing offset of the new entry that makes sure the offset of the next entry (if available) is 0.
