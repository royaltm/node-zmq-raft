...
const hasRequestExpired = (requestId, offset) => (getSeconds(requestId, offset) * 1000 < now() - REQUEST_UPDATE_TTL);
const checkRequestSanity = (requestId, offset) => {
  const nowtime = now()
...
class LogEntry extends Buffer {
  ...
  static build(requestId, type, term, data, logIndex) {
    ...
    entry.writeUIntLE(term, TERM_LOG_ENTRY_OFFSET, 7, true);
    if (isBuffer(data)) {
  ...
  get hasRequestExpired() {
    return hasRequestExpired(this);
  }

  get checkRequestSanity() {
    return checkRequestSanity(this);
  }
  ...
  readEntryTerm() {
    return this.readUIntLE(TERM_LOG_ENTRY_OFFSET, 7, true);
  }

  writeEntryTerm(term) {
    this.writeUIntLE(term, TERM_LOG_ENTRY_OFFSET, 7, true);
  }
...
defineConst(LogEntry, 'hasRequestExpired',         hasRequestExpired);
defineConst(LogEntry, 'checkRequestSanity',        checkRequestSanity);
defineConst(LogEntry, 'lastConfigEntryOf',         lastConfigEntryOf);
...
function lastConfigEntryOf(entries) {
  var i = entries.length;
  while(i--) {
    if (readTypeOf(entries[i]) === LOG_ENTRY_TYPE_CONFIG) return entries[i];
  }
}
